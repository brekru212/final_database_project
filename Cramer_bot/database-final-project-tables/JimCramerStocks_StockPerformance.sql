...
CREATE TABLE `StockPerformance` (
  ...
  `priceDifference` decimal(7,2) DEFAULT NULL,
  `days` int(11) DEFAULT '0',
  PRIMARY KEY (`ticker`),
  UNIQUE KEY `ticker` (`ticker`),
  CONSTRAINT `StockPerformance_ticker_fk` FOREIGN KEY (`ticker`) REFERENCES `CramerEntries` (`ticker`) ON DELETE CASCADE ON UPDATE NO ACTION
